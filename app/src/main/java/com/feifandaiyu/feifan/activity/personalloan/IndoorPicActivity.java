...
public class IndoorPicActivity extends BaseActivity {
    ...
    private GridImageAdapter.onAddPicClickListener onAddPicClickListener = new GridImageAdapter.onAddPicClickListener() {
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    themeStyle = ContextCompat.getColor(IndoorPicActivity.this, R.color.blue);//设置主题样式
                    checkedBoxDrawable = R.drawable.select_cb;//设置图片勾选样式
                    previewColor = ContextCompat.getColor(IndoorPicActivity.this, R.color.tab_color_true);
                    completeColor = ContextCompat.getColor(IndoorPicActivity.this, R.color.tab_color_true);
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options1).startOpenCamera(IndoorPicActivity.this, resultCallback);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options1).openPhoto(IndoorPicActivity.this, resultCallback);
                    }
                    break;
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback2);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback2);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback3);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback3);
                    }
                    break;
                case 1:
                    // 删除图片
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    themeStyle = ContextCompat.getColor(IndoorPicActivity.this, R.color.blue);
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback1);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback1);
                    }
                    break;
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback4);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback4);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback5);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback5);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback6);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback6);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback7);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback7);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(IndoorPicActivity.this, resultCallback8);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(IndoorPicActivity.this, resultCallback8);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                PictureConfig.getInstance().init(options).openPhoto(this, new ResultCa9((ImageView) view, selectMediaa, 0));
                break;
...
//                PictureConfig.getInstance().init(options).openPhoto(this, new ResultCa10((ImageView) view, selectMediab, 0));
                break;
...
//                PictureConfig.getInstance().init(options).openPhoto(this, new ResultCa11((ImageView) view, selectMediac, 0));
                break;
...
//                PictureConfig.getInstance().init(options).openPhoto(this, new ResultCa12((ImageView) view, selectMediad, 0));
                break;
            case R.id.tv_next:

                if (selectMedia11.size() <= 0) {
                    MyToast.show(this, "客户身份证正面未上传");
                    return;
                }

                if (selectMedia12.size() <= 0) {
                    MyToast.show(this, "客户身份证背面未上传");
                    return;
                }

                if (selectMedia13.size() <= 0) {
                    MyToast.show(this, "客户银行卡正面未上传");
                    return;
                }

                if (selectMedia14.size() <= 0) {
                    MyToast.show(this, "客户银行卡背面未上传");
                    return;
                }

                if (selectMedia4.size() <= 0) {
                    MyToast.show(this, "客户室内照片未上传");
                    return;
                }

                if (selectMedia3.size() <= 0) {
                    MyToast.show(this, "驾驶证明文件未上传");
                    return;
                }


                if (selectMedia.size() <= 0 && selectMedia9.size() <= 0) {
                    MyToast.show(this, "客户户口本和结婚证不能都不上传");
                    return;
                }

                if (selectMedia5.size() <= 0) {
                    MyToast.show(this, "客户房产照片未上传");
                    return;
                }

                if (selectMedia6.size() <= 0) {
                    MyToast.show(this, "客户银行流水未上传");
                    return;
                }

                if (selectMedia7.size() <= 0) {
                    MyToast.show(this, "客户APP注册截图未上传");
                    return;
                }

                if (selectMedia8.size() <= 0) {
                    MyToast.show(this, "客户代扣协议未上传");
                    return;
                }
                                            ...
                                            public void progress(String key, double percent) {
                                                ...
                                                if (percent == 1.0) {
                                                    ...
                                                    if (upLoadCount == piccount) {
                                                        String userId = PreferenceUtils.getString(IndoorPicActivity.this, "userId");
...
//                                                        System.out.println(cardsPic.toString() + "=====" + housePic.toString() + "====" + contractPic.toString());
                                                        ...
                                                        OkHttpUtils
                                                                ...
                                                                .addParams("housePic", fangchanPic.toString())
                                                                .addParams("indoor", customLivePic.toString())
                                                                .addParams("drivers", customJiashizhengPic.toString())
                                                                .addParams("household", hukouPic == null ? "-1" : hukouPic.toString())
                                                                .addParams("cards", certificate_up.toString())
                                                                .addParams("cardi", certificate_down.toString())
                                                                .addParams("marriage", customJiehunPic == null ? "-1" : customJiehunPic.toString())
                                                                .addParams("bankFlow", customBankPic.toString())
                                                                .addParams("bankcardFront", bankcard_up.toString())
                                                                .addParams("bankcardCounter", bankcard_down.toString())
                                                                .addParams("screenshot", customAppZhecePic.toString())
                                                                .addParams("appAgreement", customAppDaikouPic.toString())
                                                                .addParams("workProof", customGongzuoPic == null ? "-1" : customGongzuoPic.toString())
                                                                .build()
                                                                .execute(new StringCallback() {
                                                                    ...
                                                                    public void onResponse(String response, int id) {
                                                                        ...
                                                                        if (bean.getCode() == 1) {
                                                                            startActivity(new Intent(IndoorPicActivity.this, ContractPicActivity.class));
                                                                            finish();
                                                                            // 设置过渡动画
                                                                            ...
                                                                            upLoadCount = 0;
                                                                            MyToast.show(IndoorPicActivity.this, bean.getMsg());
                                                                        }
    ...
    protected int getContentView() {
        return R.layout.activity_indoor_pic;
    }
    ...
    private void initRecylerView() {
        ...
        hukou = (RecyclerView) findViewById(R.id.rv_hukou);
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        ...
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia5);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia5.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia5.get(position - 1).getPath());
                        }
                        break;
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia3);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia3.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia3.get(position - 1).getPath());
                        }
                        break;
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia4);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia4.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia4.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter4.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia6);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia6.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia6.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter5.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia7);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia7.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia7.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter6.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia8);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia8.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia8.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter7.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia9);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia9.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia9.get(position - 1).getPath());
                        }
                        break;
        ...
        adapter8.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    ...
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        PictureConfig.getInstance().externalPicturePreview(IndoorPicActivity.this, position - 1, selectMedia10);
                        break;

                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia10.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(IndoorPicActivity.this, selectMedia10.get(position - 1).getPath());
                        }
                        break;
    ...
    class ResultCa9 implements PictureConfig.OnSelectResultCallback {
        ...
        public void onSelectSuccess(List<LocalMedia> resultList) {
            ...
            if (media.isCut() && !media.isCompressed()) {
                ...
                Glide.with(IndoorPicActivity.this)
                        .load(path1)
                        .centerCrop()
