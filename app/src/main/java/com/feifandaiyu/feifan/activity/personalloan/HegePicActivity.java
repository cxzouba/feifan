...
public class HegePicActivity extends BaseActivity {
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    themeStyle = ContextCompat.getColor(HegePicActivity.this, R.color.blue);//设置主题样式
                    checkedBoxDrawable = R.drawable.select_cb;//设置图片勾选样式
                    previewColor = ContextCompat.getColor(HegePicActivity.this, R.color.tab_color_true);
                    completeColor = ContextCompat.getColor(HegePicActivity.this, R.color.tab_color_true);
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(HegePicActivity.this, resultCallback2);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(HegePicActivity.this, resultCallback2);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(HegePicActivity.this, resultCallback3);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(HegePicActivity.this, resultCallback3);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(HegePicActivity.this, resultCallback1);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(HegePicActivity.this, resultCallback1);
                    }
                    break;
                case 1:
                    // 删除图片
                                            ...
                                            public void progress(String key, double percent) {
                                                ...
                                                if (percent == 1.0) {
                                                    ...
                                                    if (upLoadCount == piccount) {
                                                        String carId = PreferenceUtils.getString(HegePicActivity.this, "carId");

//                                                        System.out.println(cardsPic.toString() + "=====" + groupPhoto.toString() + "====" + contractPic.toString() + "====" + other==null?"-1":other.toString());

                                                        OkHttpUtils
                                                                .post()
                                                                .url(Constants.URLS.BASEURL + "Mortgage/Certificate")
                                                                .addParams("carId", carId)
                                                                .addParams("certificate", cardsPic.toString())//合格证（新车）必填
                                                                .addParams("insurance", groupPhoto.toString())//交强险保单 非必填
                                                                .addParams("invoice", contractPic.toString())//发票（新车） 非必填
...
//                                                                            startActivity(new Intent(HegePicActivity.this, DiyaPicActivity.class));
                                                                            ...
                                                                            MyToast.show(HegePicActivity.this, bean.getMsg());
                                                                        }
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        ...
                        PictureConfig.getInstance().externalPicturePreview(HegePicActivity.this, position - 1, selectMedia5);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia5.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(HegePicActivity.this, selectMedia5.get(position - 1).getPath());
                        }
                        break;
                }
            }
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(HegePicActivity.this, position - 1, selectMedia3);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia3.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(HegePicActivity.this, selectMedia3.get(position - 1).getPath());
                        }
                        break;
                }
            }
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(HegePicActivity.this, position - 1, selectMedia);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(HegePicActivity.this, selectMedia.get(position - 1).getPath());
                        }
                        break;
                }
