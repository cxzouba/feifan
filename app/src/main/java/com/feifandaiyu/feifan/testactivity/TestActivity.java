...
public class TestActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initRecyclerView() {
        ...
        adapter1 = new GridImageAdapter(TestActivity.this, onAddPicClickListener1);
        adapter1.setList(selectMedia);
        adapter1.setSelectMax(maxSelectNum);
        FullyGridLayoutManager manager = new FullyGridLayoutManager(TestActivity.this, 3, GridLayoutManager.VERTICAL, false);
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        ...
                        PictureConfig.getInstance().externalPicturePreview(TestActivity.this, position, selectMedia);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(TestActivity.this, selectMedia.get(position - 1).getPath());
                        }
                        break;
                }
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    themeStyle = ContextCompat.getColor(TestActivity.this, R.color.blue);//设置主题样式
                    checkedBoxDrawable = R.drawable.select_cb;//设置图片勾选样式
                    previewColor = ContextCompat.getColor(TestActivity.this, R.color.tab_color_true);
                    completeColor = ContextCompat.getColor(TestActivity.this, R.color.tab_color_true);
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(TestActivity.this, resultCallback);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(TestActivity.this, resultCallback);
                    }
                    break;
                case 1:
                    // 删除图片
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tv_next) {
            if (selectMedia.size() <= 0) {
                MyToast.show(TestActivity.this, "请上传照片");
            } else {
                postMessage();
            }
        } else if (v.getId() == R.id.iv_back) {
            new AlertDialog.Builder(this)
                    .setMessage("未新增照片是否返回")
    ...
    private void postMessage() {
        ...
        for (int i = 0; i < medias.size(); i++) {
            ...
            for (int j = 0; j < localMedias.size(); j++) {
                compressPath = localMedias.get(j).getCompressPath();
                ...
                if (finalI == 0) {
                    ...
                    try {
                        ...
                        PreferenceUtils.setString(TestActivity.this, "imagearray", image.toString());
                        PreferenceUtils.setString(TestActivity.this, "imagecount", selectMedia.size() + "张");
