...
public class MyCreatAdapter2 extends BaseAdapter {

    private Context context;
    private UpdateCreatBean bean;

    public MyCreatAdapter2(Context context, UpdateCreatBean bean) {
    ...
    @Override
    public int getCount() {
        return bean.getList().getOverduepic() == null ? 0 : bean.getList().getOverduepic().size();
    }

    @Override
    public Object getItem(int position) {
        return bean.getList().getOverduepic().get(position);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        ImageViewUtils.showNetImage(context,bean.getList().getOverduepic().get(position),R.drawable.crabgnormal,holder.fiv);

        holder.ivDel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                bean.getList().getOverduepic().remove(position);
        ...
        holder.fiv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("image01", bean.getList().getOverduepic().get(position).split(",")[0]);
