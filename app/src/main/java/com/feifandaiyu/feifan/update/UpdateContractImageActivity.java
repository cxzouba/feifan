...
import com.feifandaiyu.feifan.adapter.MyCustomPaperAdapter;
import com.feifandaiyu.feifan.adapter.MyMianqianAdapter;
import com.feifandaiyu.feifan.adapter.MyShouchiAdapter;
...
public class UpdateContractImageActivity extends BaseActivity {
    ...
    private ContractBean updatePersonalPicBean;
    ...
    protected int getContentView() {
        ...
        return R.layout.activity_update_contract_image;
    ...
    private void initData() {
        ...
        post()
                .url(Constants.URLS.BASEURL + "UserPic/contractPicShow")
                .addParams("userId", userId)
                .build()
                .execute(new StringCallback() {
                    @Override
                    public void onError(Call call, Exception e, int id) {
                        LogUtils.i("updatecontractPicShow----------------->>>>>>." + e);
//                        hud.dismiss();
                        MyToast.show(UpdateContractImageActivity.this, "联网失败...");
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        LinearLayoutManager ms1 = new LinearLayoutManager(UpdateContractImageActivity.this);
                        ms1.setOrientation(LinearLayoutManager.HORIZONTAL);
                        LinearLayoutManager ms2 = new LinearLayoutManager(UpdateContractImageActivity.this);
                        ms2.setOrientation(LinearLayoutManager.HORIZONTAL);
                        LinearLayoutManager ms3 = new LinearLayoutManager(UpdateContractImageActivity.this);
                        ms3.setOrientation(LinearLayoutManager.HORIZONTAL);

                        updatePersonalPicBean = gson.fromJson(json, ContractBean.class);
...
//                        hud.dismiss();
                        ...
                        if (updatePersonalPicBean.getCode() == 1) {
                            ...
                            rvMianqian.setAdapter(new MyMianqianAdapter(updatePersonalPicBean, UpdateContractImageActivity.this));
                            rvShouchi.setLayoutManager(ms2);
                            rvShouchi.setAdapter(new MyShouchiAdapter(updatePersonalPicBean, UpdateContractImageActivity.this));
                            rvCustomPaper.setLayoutManager(ms3);
                            rvCustomPaper.setAdapter(new MyCustomPaperAdapter(updatePersonalPicBean, UpdateContractImageActivity.this));

                            etContractNum.setText(updatePersonalPicBean.getContractCode());
                            etContractNum.setEnabled(false);
                            tvAddr.setText(updatePersonalPicBean.getSignedPlace());

                            etTailongNum.setText(updatePersonalPicBean.getTlContractCode());

                        } else if (updatePersonalPicBean.getCode() == 0) {
                            new AlertDialog.Builder(UpdateContractImageActivity.this)
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        ...
                        PictureConfig.getInstance().externalPicturePreview(UpdateContractImageActivity.this, position - 1, selectMedia5);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia5.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(UpdateContractImageActivity.this, selectMedia5.get(position - 1).getPath());
                        }
                        break;
                }
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(UpdateContractImageActivity.this, position - 11, selectMedia3);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia3.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(UpdateContractImageActivity.this, selectMedia3.get(position - 1).getPath());
                        }
                        break;
                }
            ...
            public void onItemClick(int position, View v) {
                switch (selectType) {
                    case FunctionConfig.TYPE_IMAGE:
                        // 预览图片 可长按保存 也可自定义保存路径
                        //PictureConfig.getInstance().externalPicturePreview(MainActivity.this, "/custom_file", position, selectMedia);
                        PictureConfig.getInstance().externalPicturePreview(UpdateContractImageActivity.this, position - 1, selectMedia);
                        break;
                    case FunctionConfig.TYPE_VIDEO:
                        // 预览视频
                        if (selectMedia.size() > 0) {
                            PictureConfig.getInstance().externalPictureVideo(UpdateContractImageActivity.this, selectMedia.get(position - 1).getPath());
                        }
                        break;
                }
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    themeStyle = ContextCompat.getColor(UpdateContractImageActivity.this, R.color.blue);//设置主题样式
                    checkedBoxDrawable = R.drawable.select_cb;//设置图片勾选样式
                    previewColor = ContextCompat.getColor(UpdateContractImageActivity.this, R.color.tab_color_true);
                    completeColor = ContextCompat.getColor(UpdateContractImageActivity.this, R.color.tab_color_true);
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(UpdateContractImageActivity.this, resultCallback1);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(UpdateContractImageActivity.this, resultCallback1);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(UpdateContractImageActivity.this, resultCallback2);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(UpdateContractImageActivity.this, resultCallback2);
                    }
                    break;
                case 1:
                    // 删除图片
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(UpdateContractImageActivity.this, resultCallback3);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(UpdateContractImageActivity.this, resultCallback3);
                    }
                    break;
                case 1:
                    // 删除图片
                        ...
                        for (int j = 0; j < localMedias.size(); j++) {
                            ...
                            if (finalI == 0) {
                                ...
                                try {
                                    ...
                                    updatePersonalPicBean.getInterview().getList().add(key);
                                ...
                                try {
                                    ...
                                    updatePersonalPicBean.getHold().getList().add(key);
                                } catch (JSONException e) {
                                    e.printStackTrace();
                                ...
                                try {
                                    ...
                                    updatePersonalPicBean.getContractPic().getList().add(key);
                                } catch (JSONException e) {
                                    e.printStackTrace();
    ...
    private void post2() {
        ...
        Gson gson = new Gson();
        String mianQian = gson.toJson(updatePersonalPicBean.getInterview().getList());
        String shouchi = gson.toJson(updatePersonalPicBean.getHold().getList());
        String contractPic = gson.toJson(updatePersonalPicBean.getContractPic().getList());

        System.out.println(mianQian.toString() + "=====" + shouchi.toString() + "====" + contractPic.toString());

        post()
                .url(Constants.URLS.BASEURL + "UserPic/contractPic")
                .addParams("userId", userId)
                .addParams("interview", mianQian.toString())
                .addParams("contractPic", contractPic.toString())
                .addParams("hold", shouchi.toString())
                .addParams("contractCode", etContractNum.getText().toString().trim())
                .addParams("saleId", saleID)
                .addParams("signedPlace", tvAddr.getText().toString())
...
//                          startActivity(new Intent(UpdatePersonalImageActivity.this, UpLoadCusomerVideoActivity.class));
                            MyToast.show(UpdateContractImageActivity.this, "合同照片保存成功");
                            ...
                            MyToast.show(UpdateContractImageActivity.this, bean.getMsg());
                        }
                    }
                });
    }
