...
import com.feifandaiyu.feifan.adapter.MyCreatAdapter;
import com.feifandaiyu.feifan.adapter.MyCreatAdapter2;
...
public class UpdateCustomeReport extends BaseActivity implements View.OnClickListener {
...
//    RecyclerView recyclerView;
    ...
    LinearLayout llYuqi;
    @InjectView(R.id.mygridview2)
    CustomGridView mygridview2;
...
//    RecyclerView rvYuqi;
    ...
    private UpdateCreatBean updateReportBean;
    private int credit;
    private String url;
    private String type;
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        hud = KProgressHUD.create(UpdateCustomeReport.this)
        ...
        post()
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int id) {
                        LogUtils.i("updateReportShow----------------->>>>>>." + e);
//                        hud.dismiss();
                        MyToast.show(UpdateCustomeReport.this, "联网失败...");
                    }

                    @Override
                    public void onResponse(String response, int id) {
                        LogUtils.i("updateReportShow----------------->>>>>>." + response);
                        String json = response;
                        Gson gson = new Gson();

                        try {
                            updateReportBean = gson.fromJson(json, UpdateCreatBean.class);
                        } catch (JsonSyntaxException e) {
                            e.printStackTrace();
                            MyToast.show(UpdateCustomeReport.this, "json转换异常");
                        }
//                        hud.dismiss();

                        if (updateReportBean.getCode() == 1) {

                            credit = updateReportBean.getList().getCredit();
                            if (credit == 0) {
                            ...
                            etCustomerName.setText(updateReportBean.getList().getUserName());
                            etCertificateNum.setText(updateReportBean.getList().getCardNum());
                            etJiashiNum.setText(updateReportBean.getList().getDrivingLicence());
                            remark.setText(updateReportBean.getList().getOverdue().equals("-1") ? "无" : updateReportBean.getList().getOverdue());

                            if (updateReportBean.getList().getOverduepic().get(0).equals("-1")) {
                                llYuqi.setVisibility(View.GONE);
                            } else {

                                mygridview2.setAdapter(new MyCreatAdapter2(UpdateCustomeReport.this, updateReportBean));

                            }

                            mygridview.setAdapter(new MyCreatAdapter(UpdateCustomeReport.this, updateReportBean));

                        } else if (updateReportBean.getCode() == 0) {
                            new AlertDialog.Builder(UpdateCustomeReport.this)
...
//        adapter1 = new GridImageAdapter(UpdateCustomeReport.this, onAddPicClickListener1);
//        adapter1.setList(selectMedia);
//        adapter1.setSelectMax(maxSelectNum);
//        FullyGridLayoutManager manager = new FullyGridLayoutManager(UpdateCustomeReport.this, 3, GridLayoutManager.VERTICAL, false);
...
//                        PictureConfig.getInstance().externalPicturePreview(UpdateCustomeReport.this, position, selectMedia);
//                        break;
//                    case FunctionConfig.TYPE_VIDEO:
//                        // 预览视频
//                        if (selectMedia.size() > 0) {
//                            PictureConfig.getInstance().externalPictureVideo(UpdateCustomeReport.this, selectMedia.get(position - 1).getPath());
//                        }
//                        break;
//
//                    default:
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    themeStyle = ContextCompat.getColor(UpdateCustomeReport.this, R.color.blue);//设置主题样式
                    checkedBoxDrawable = R.drawable.select_cb;//设置图片勾选样式
                    previewColor = ContextCompat.getColor(UpdateCustomeReport.this, R.color.tab_color_true);
                    completeColor = ContextCompat.getColor(UpdateCustomeReport.this, R.color.tab_color_true);
...
//                            .setImmersive(false)// 是否改变状态栏字体颜色(黑色)
                    ...
                    if (mode) {
                        // 只拍照
                        PictureConfig.getInstance().init(options).startOpenCamera(UpdateCustomeReport.this, resultCallback);
                    } else {
                        // 先初始化参数配置，在启动相册
                        PictureConfig.getInstance().init(options).openPhoto(UpdateCustomeReport.this, resultCallback);
                    }
                    break;
                case 1:
                    // 删除图片
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.tv_next) {
            ...
            if (credit == 0) {
                if (updateReportBean.getList().getPic().size() + selectMedia.size() > 0) {
                ...
                if (updateReportBean.getList().getPic().size() + selectMedia.size() > 4) {

                    upLoad();

                } else {
                    MyToast.show(this, "银行授权书图片不能少于5张");
                }
            }
    ...
    private void upLoad() {
        ...
        if (piccount == 0) {
            ...
            for (int j = 0; j < selectMedia.size(); j++) {
                ...
                updateReportBean.getList().getPic().add(key);
...
//
        ...
        String userId = PreferenceUtils.getString(UpdateCustomeReport.this, "userId");

        post()
                .url(Constants.URLS.BASEURL + url)
                .addParams("userId", userId)
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int id) {
                        ...
                        MyToast.show(UpdateCustomeReport.this, "服务器链接失败，请联系管理员");
                    }
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            MyToast.show(UpdateCustomeReport.this, "服务器连接失败");
                        }
...
//                            startActivity(new Intent(UpdateCustomeReport.this, StartHomeVisitActivity.class));
                            MyToast.show(UpdateCustomeReport.this, "征信照片保存成功");
                            finish();
                            int enterAnim0 = R.anim.pre_enter;// 进入的activity对应的动画资源
                            int exitAnim0 = R.anim.pre_exit;// 结束的activity对应的动画资源
                            overridePendingTransition(enterAnim0, exitAnim0);
                            ...
                            MyToast.show(UpdateCustomeReport.this, customeReportBean.getMsg());
                        }
